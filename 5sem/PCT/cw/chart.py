...
def draw(filenames, labels):
    ...
    ax.set_title("")
    ax.set(xlabel="Number p processes (nodes x processes)", ylabel="Speedup S(p)")
    ax.label_outer()
    ...
    for i in range(0, len(dataX)):
        ...
        for j in range(len(dataX[i])):
            ax.annotate(str(dataY[i][j]), xy=(dataX[i][j] + p_offset - 0.3, dataY[i][j]), ha='right', va='bottom', fontsize=5)
...
if __name__ == "__main__":
    draw(["27.out", "40.out"], ["N = 27000", "N = 40000"])