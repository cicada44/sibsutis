...
void *bstreeAdd(struct bstree *tree, char *key, int value){
    ...
    if (strcmp(key,parent->key)<0) parent->left = node;
    else parent->right = node;
}

struct bstree *bstreeLookup(struct bstree *tree, char *key){
    if (tree == NULL) return NULL;
    while (strcmp(key,tree->key)!=0){
        if (strcmp(key,tree->key)>0) tree = tree->right;
        else tree = tree->left;
    }
    return tree;
}
...
struct bstree *bstreeFindParent(struct bstree *tree, char *key){
    ...
    while (parent->left != NULL && parent->right != NULL){
        if (strcmp(parent->left->key, key) == 0 || strcmp(parent->right->key, key) == 0) return 0;
        else if (strcmp(parent->key, key) > 0) parent = parent->left;
        else if (strcmp(parent->key, key) < 0) parent = parent->right;
        else return NULL;
    }
    return parent;
}
